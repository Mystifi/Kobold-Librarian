...
quills.addShopItem('bio', "Personalized Bio", 500, `A personalized bio <a href="bios.html">on ${config.username}'s website</a>. Use <code>${config.commandToken}setbio text</code> to set your bio. Bios use Markdown formatting. Inappropriate bios will get your bio removed and purchase revoked.`, null, true);
...
server.addRoute(`/public-page.html`, (url, queryData, tokenData, res) => {
	...
	return [pageid, `<form method="post"><textarea style=width:100%;" rows=50 name="content">${storage.getJSON('public-pages')[pageid] ? storage.getJSON('public-pages')[pageid].split('\n').slice(0, -1).join('\n') : ''}</textarea><input type="submit" value="Submit changes"/></form>`];
